...
resource "aws_instance" "ec2-public-management-pops" {
  ...
  key_name                    = aws_key_pair.public_key_ec2_management.key_name
  ...
  tags = {
    Name = "ec2-public-squad-management-pops"
  }
}

resource "aws_key_pair" "public_key_ec2_analysis" {
  key_name = var.key_pair_name_analysis_public
  public_key = file("${path.root}/keys/key-ec2-data-analysis-pops.pem.pub")
}

resource "aws_instance" "ec2-public-data-analysis-pops" {
  ami                         = "ami-0e86e20dae9224db8"
  instance_type               = var.instance_type
  subnet_id                   = var.subnet_public_id
  associate_public_ip_address = true
  ...
  iam_instance_profile        = "LabInstanceProfile"
  key_name                    = aws_key_pair.public_key_ec2_analysis.key_name
  ...
  provisioner "remote-exec" {
    inline = [
      "chmod +x /home/ubuntu/public_data_analysis.sh",
    ]
  }
  ...
  tags = {
    Name = "ec2-public-data-analisys-pops"
  }
}
...
depends_on = [aws_instance.ec2-public-management-pops]
